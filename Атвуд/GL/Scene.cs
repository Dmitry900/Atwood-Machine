...
namespace Atwood
{
    class Camera
    {
        ...
        private Vector3D Cross(Vector3D vV1, Vector3D vV2, Vector3D vVector2)
        {
            ...
            vVector1.z = vV1.z - vV2.z; // Если у нас есть 2 вектора (вектор взгляда и вертикальный вектор), 
        ...
        public void Scale(float speed)
        {
            ...
            mPos.x += (mPos.x * speed - mView.x * speed) / 10;
            mPos.z += (mPos.z * speed - mView.z * speed) / 10;
            mPos.y += (mPos.y * speed - mView.y * speed) / 10;
            //mUp.x += 1*speed / 10;
            //mUp.y += 1*speed / 10;
        }
